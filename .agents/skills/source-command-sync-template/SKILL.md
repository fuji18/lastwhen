...
3. `.claude/hooks/` `.claude/scripts/` `.claude/settings.json` に変更があった場合、**反映には Codex の再起動が必要**である旨を伝える。
...
- **`never` のファイルは理由があっても触らない。** テンプレート側の AGENTS.md / README / docs はプロジェクトが書き換え済みで、上書きするとプロダクトの情報が失われる。テンプレート側の AGENTS.md にルールが増えていた場合、それは `.claude/rules/` に切り出されていないテンプレート側の設計ミスなので、**取り込まずにテンプレートへフィードバックする**
