...
- `claude/[自動生成名]`: Claude Code のアプリ/web リモートセッションがプラットフォーム側で生成するブランチ。`feature/*` と同格の正規ブランチとして扱い、**リネームしない**(セッションとの紐付けが壊れる)
...
**機械可読な単一ソース**: ベースブランチ・許可接頭辞・保護ブランチは `.claude/branch-policy.json` に定義する。この文書は人間向けの説明であり、**hook・CI・スラッシュコマンドが参照する値はポリシーファイルが正**。両者を変更するときは必ず同時に更新する。
