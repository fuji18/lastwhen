...
└── develop (開発・統合環境)
    ├── feature/* (新機能開発)
    ├── fix/* (バグ修正)
    ├── claude/* (アプリ/web リモートセッションが生成。feature/* と同格)
...
- **claude/\***: Claude Code のアプリ/web セッションがプラットフォーム側で先に作るブランチ。**リネームせず**そのまま作業ブランチとして使い、PR のベースだけ `develop` に揃える(名前ではなく「どこから切ってどこへ PR するか」を守る)
...
以下はテンプレート既定スタック(Node.js/TypeScript)での構成例。異なるスタックのプロジェクトでは、同じ観点(lint / フォーマット / 型チェック / テスト / ビルド)を実際のツールチェーンに読み替える。

**自動化項目と採用ツール**:

1. **Lintチェック**
   - **ESLint 10.x** + **@typescript-eslint**
