...
- 優先度(P0/P1/P2)の判断は**司令塔の担当**です。`design.md` が `<!-- status: ready -->` で渡された時点で、**そのチケットへの着手は承認済み**として扱ってください(チケット自体が P1/P2 であることを理由に停止しないでください。P0 の完了状況はあなたからは確認できず、確認できた司令塔が渡しています)
- 禁止するのは**チケットの範囲を超えた前倒し**です。`design.md` が P1/P2・将来対応・範囲外としている機能は実装しないでください
...
- `AGENTS.md` — このファイル自身。冒頭の verify-probe コメントは次回の委託時にホスト側で読まれます。既定の exists 形式はプロセスを起動しませんが、**後方互換の形式(npx / python3 等)に書き換えられるとホスト上でコマンドが実行されます**。形式検査が入っていますが、**検査を通る範囲でも書き換えないでください**
...
**このプロジェクト固有の禁止領域**(/kickoff フェーズ4 で追加):

- `lib/data/database/` — Drift のスキーマ定義とテーブル定義の実体です。列の型や制約が 1 つ変わるだけで、既存ユーザーの記録が読めなくなります。このアプリはローカル完結でサーバー側にバックアップが無いため、壊れた場合の復旧手段がありません
- `lib/data/migrations/` — マイグレーション定義です。**一度ストアへ出荷したマイグレーションは修正できません**。誤った移行を配信すると、更新したユーザーの記録が不可逆に失われます。スキーマ変更は司令塔または fork が直接書き、旧バージョンからの移行テストを必ず添えます
<!-- /kickoff:delegation-forbidden-paths -->
