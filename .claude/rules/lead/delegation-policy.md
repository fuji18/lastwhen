...
> **このプロジェクトでは、委託先はテストを回せない。** `flutter test` は 127.0.0.1 の
> サーバーソケットを作るが sandbox は `network_access = false` で、ループバックだけを
> 許可する設定は codex-cli に無い。**format と analyze までは委託先が自力で通す**
> (`AGENTS.md` §2 が SDK ルートへ書かない Dart SDK 直叩きを指示している)。
> **テストは検収側(`/check` / CI)が回す**のが正規の分担であって、委託の失敗ではない。
