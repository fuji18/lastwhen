...
>
> **スコープ判断(司令塔 / 2026-09-23)**: 本チケットは P1 だが、**P0 チケットは全件 closed(MVP 完了)**で
> 「前倒し」には当たらない。P1 の #20 も同じ扱いで実装・マージ済み。`AGENTS.md` §4 の「P0 以外を実装しない」は
> MVP 期間中の前倒し禁止であり、**このチケットの実装は承認済み**として進めてよい(この判断で停止しないこと)。
