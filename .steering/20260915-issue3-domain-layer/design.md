...
| 日付をまたぐと 24 時間未満でも 1 日 | **ローカル**リテラル last = 2026-01-10 23:00 / now = 2026-01-11 02:00 | `1` |
| 同じ日の 23:59 は 0 日 | **ローカル**リテラル last = now = 2026-01-10 23:59 | `0` |
...
> **タイムゾーン非依存にするための書き分け**(レビュー指摘 #3-1 を受けた修正):
>
> - **両方の時刻帯が同じケースは UTC リテラル**(`DateTime.utc(..., 6, 0)` など)。
>   タイムゾーンが変わると両方の暦日が一様にずれるだけなので、**差は保たれる**
> - **時刻帯が異なるケース(深夜をまたぐ検証)は、ローカルリテラル `DateTime(...)` で書く。**
>   `elapsedDays` は内部で `.toLocal()` するため、ローカルリテラルでは恒等変換になり、
>   書いたとおりの暦日で評価される。UTC リテラルで `23:00 → 翌 02:00` と書くと、
>   JST では両方が同じ暦日(1/11 08:00 と 11:00)になり **0 が返って落ちる**(実測)
>
> 実行環境は devcontainer / CI = UTC、開発者の端末 = JST。**`TZ=Asia/Tokyo flutter test` でも
> 通ることを確認する。**
...
flutter test
TZ=Asia/Tokyo flutter test   # 開発者の端末は JST。タイムゾーン依存のテストを弾く
```
