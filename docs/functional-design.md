...
- これは**既定の並び**。一覧で別の並びを選べる(下記 F15)。図鑑は常にこの並び

### 一覧の並び替え(F15)

一覧の AppBar の並び替えボタン(`Icons.sort`、ツールチップ「並び順」)からメニューを開き、並び順を選ぶ。
選択中の候補にはチェックを付ける。ボタンは一覧に項目が 1 件以上あるときだけ出す(追加の FAB と同じ条件)。

| 並び順 | UI 文言 | 並べ方 | 未実施の位置 | 同値 |
| --- | --- | --- | --- | --- |
| 既定(F30) | 経年順 | 相対経過度の降順 | 相対経過度が null の項目と一緒に下部 | 登録順 |
| 経過日数 | 経過日数順 | 経過日数の降順(長く空いている項目が上) | **下部にまとめる** | 登録順 |
| 名前 | 名前順 | 項目名の昇順(下記の比較) | 区別しない(名前の位置に並ぶ) | 登録順 |
| 登録 | 登録順 | `sortOrder` の昇順(古い項目が上) | 区別しない | — |

- **記録操作では並びを組み替えない(F30 の規則を全ての並び順で保つ)。** 並びを確定させるのは F30 と同じ
  タイミング(一覧の表示・再試行・前面への復帰)と、**並び順を選び直したとき**。選び直したときは
  その時点の時刻で経過日数を数え直す。いま選んでいる並び順をもう一度選んでも何も起きない
- 改名しても名前順の位置は変わらない(確定済みの並びを保つ)。次の確定で反映する
- **選択は保存しない。** 再起動すると経年順に戻る(F13 と同じ方針。ユーザー設定の保存先を持たない)
- カテゴリの絞り込み(F13)は、選んだ並びの**部分列**として表示する
- 図鑑(F31)の並びは一覧の選択に追従しない
- 並べ替えは F30 と同じく `ItemListNotifier` のメモリ上で行う。データ層のクエリは変えない

**名前順の比較**: 読み仮名のデータを持たないため、項目名を次の順に正規化したキーを**文字コード順**で比べる。

1. 全角英数字・記号(U+FF01〜U+FF5E)を半角(U+0021〜U+007E)へ、全角スペース(U+3000)を半角スペースへ
2. 英字を小文字へ
3. カタカナ(U+30A1〜U+30F6)をひらがな(U+3041〜U+3096)へ

- かな同士は五十音順(清音の直後に濁音・半濁音)に並ぶ。「ゴミ出し」と「ごはん」は隣り合う
- **漢字は読みではなく文字コード順**で、かなより後ろに来る(制約。読み仮名の入力を持つまで解消しない)
- 半角カタカナは正規化しない(入力で使われることが稀なため)。長音符「ー」はそのまま

### カテゴリの絞り込み(F13)

一覧上部にカテゴリのチップ列(先頭は「すべて」)を置き、選んだカテゴリの項目だけを表示する。

- **絞り込みは表示の上での部分列。** 選んだ並び(既定は F30)を絞り込み後も保つ
...
- 並びは**常に F30**(確定のタイミングも一覧と同じ)。一覧で選んだ並び順(F15)には追従しない。
  図鑑専用の並び替えは持たない
...
| 並び順(F30 / F15) | 並び順ごとの並べ方・未実施の位置・同値の登録順。名前順のキーの正規化(全角→半角・大文字→小文字・カタカナ→ひらがな) |
...
| 並び順の切り替え | メニューで選んだ順に並び、記録しても組み替わらず、絞り込みと併用しても選んだ並びを保つ。図鑑は経年順のまま |
