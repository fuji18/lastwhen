...
| `lastDoneAt` | NULL 許容。**NULL = 一度も記録がない**(F4 の「未実施」表示の根拠)。**`done_logs` の `MAX(done_at)` と常に一致させる**(最新ログのキャッシュ)。過去の日付で記録(F16)しても、それが最新でなければ動かない |
...
abstract interface class ItemRepository {
  ...
  /// 過去の日付で記録する(F16)。`done_logs` に 1 行追加し、`lastDoneAt` を
  /// `MAX(done_at)` に揃える。`updatedAt` は [now]。同一トランザクション。
  /// 追加した履歴の ID を返す(取り消しに使う)。対象が無ければ書かずに null
  Future<DoneLogId?> addDoneLog(ItemId id, DateTime doneAt, {required DateTime now});

  /// addDoneLog の取り消し。指定した履歴 1 行だけを消し、`lastDoneAt` を残りの
  /// `MAX(done_at)`(無ければ null)に揃える。同一トランザクション
  Future<void> removeDoneLog(ItemId id, DoneLogId logId, {required DateTime now});
}
```
...
> **F16(過去の日付で記録)は `doneAt` が過去日になる**ので、`updatedAt` を別に受け取る
> `addDoneLog` を足した(#49)。当初の案の `setLastDoneAt`(最終実施日の置き換え)は採らない。
> 置き換えると記録忘れの後追いで直近の実施が消え、基準間隔の学習から 1 回分が欠ける。
> 取り消しは「直近 1 行」ではなく**追加した 1 行を ID で**消す(過去日の行は直近とは限らない)。
...
1. 詳細シートの「日付を指定して記録」を押す。シートを閉じ、日付の選択を開く(初期値は今日、最終日は今日)
2. 日付を選んで「記録する」を押す。キャンセルなら何もしない
3. Notifier は選んだ暦日を記録日時に直す: **今日なら現在時刻、過去日ならその日のローカル 12:00**
   (0:00 にするとタイムゾーンを西へまたいだとき前日に読める。正午なら ±12 時間まで同じ暦日に収まる)
4. リポジトリが `done_logs` に 1 行追加し、`lastDoneAt` を `MAX(done_at)` に揃える。
   **選んだ日が既存の最終実施日より古ければ `lastDoneAt` は動かない**(履歴と基準間隔だけが変わる)
5. 同じ暦日の記録が既にあっても追加する。基準間隔の計算は暦日単位でまとめるため 0 日間隔は生まれない
6. 成功したら `M月d日で記録しました` と取り消し導線を 4 秒出す。取り消すと追加した 1 行だけを消す。
   並びは組み替えない(F30)
7. 書き込みが失敗したら「保存できませんでした。もう一度お試しください」を出す
...
**通常操作の画面は 3 つ**(一覧・登録・編集)と、一覧に重ねる詳細シート(F29)。
一覧と図鑑(F31)は下部ナビで切り替える兄弟の画面で、起動直後は必ず一覧(ホーム)に出る。
図鑑の詳細シートから編集へ進んだ後、保存・キャンセルで戻る先は図鑑になる。記録は遷移を伴わない —— これが「1 タップ」の実体。
...
- 「編集」の左に「日付を指定して記録」(F16)を置く。押すとシートを閉じて日付の選択(`showDatePicker`)を開く。
  選べるのは今日まで(未来日は選べない)。選ぶと記録し、`M月d日で記録しました` と取り消し導線を
  **4 秒間**出す(「やった」と同じ)。キャンセルなら何もしない。**確認ダイアログは出さない**
  (日付を選んで決定する操作そのものが意思表示で、誤りは取り消しで救う)。
  一覧・図鑑のどちらから開いたシートにも出す
...
下部ナビ(`NavigationBar`)の「ホーム / 図鑑」で一覧と切り替える。**1 回以上記録した項目**のカードを
グリッドで並べる。

- **載る条件は「未実施でないこと」。** 一覧との違いを作るのはここで、未実施の項目は図鑑に載らない
- 並びは一覧と同じ(F30 の確定済みの並び)。図鑑専用の並び替えは持たない
- カードは「アイコン・項目名・`平均7日`(基準間隔を四捨五入)」。基準間隔が無ければ `学習中`。
  紙の古び(経年ステージ)は一覧のカードと同じに掛ける
- **記録(「やった」)の入口を置かない。** カードのタップは詳細シート(F29)を開くだけ。
  この決まりはカード上の 1 タップの記録を指す。詳細シートの「日付を指定して記録」(F16)は
  一覧から開いたときと同じく出す(#49)
- 上部にカテゴリのチップ列(F13 と同じ規則)。**選択は一覧と独立**で、保存しない
- AppBar の検索ボタンで項目名の検索欄に切り替える。部分一致・英字の大文字小文字を区別しない。
  検索を閉じると条件を消す。検索中はシステムの戻る(Android の戻るボタン・ジェスチャー)でも
  検索を閉じる(アプリを終了させない)。図鑑タブが表示されていないときは戻るを横取りしない
- 記録済みの項目が 0 件なら「まだ図鑑にカードがありません」、絞り込み・検索の結果が 0 件なら
  「条件に合う項目はありません」。図鑑には項目の追加導線を置かない
- タブを切り替えるときは取り消し導線を閉じる(画面遷移と同じ理由)
- マイページは中身が決まるまで置かない
...
| 記録直後 | カードが `今日` に変わり、取り消し導線が **4 秒間**出る(`SnackBar` の既定)。直近 1 件のみ。**カードの位置は動かない**(F30) |
...
- **一覧・図鑑は遅延生成する**(`ListView.builder` / `GridView.builder`)。100 件で全カードを同時に構築しない
...
- **Drift のクエリは `watchAll` の 1 本**に絞る。カードごとの個別クエリを出さない。図鑑も一覧と同じ `itemListProvider` から絞り込んで作り、購読を増やさない
...
| 過去日の記録(最新より古い) | 履歴に追加され、`lastDoneAt` は動かず、`updatedAt` は `now` で進む |
| 過去日の記録(最新より新しい / 未実施) | `lastDoneAt` がその日時になる |
| 過去日の記録 → 取り消し | 追加した 1 行だけが消え、`lastDoneAt` が残りの最大値(無ければ null)に戻る |
...
| 日付を指定して記録 | 詳細シートから日付を選ぶと記録され、未来日は選べず、取り消せる |
